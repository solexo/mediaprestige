  ...
  return (
    <>
      ...
      <button
        ...
        className="fixed bottom-6 right-6 bg-yellow-400 hover:bg-yellow-500 text-black p-4 rounded-full shadow-2xl transition-all duration-300 hover:scale-110 z-50"
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            {messages.map((message) => (
              <div key={message.id} className={`flex ${message.isBot ? 'justify-start' : 'justify-end'}`}>
                <div className={`max-w-[80%] ${message.isBot ? 'bg-gray-800' : 'bg-yellow-400'} rounded-2xl px-4 py-2`}>
                  {message.image && (
                    <div className="mb-3">
                      <img
                        src={message.image}
                    ...
                    <div className="flex flex-wrap gap-2 mt-3">
                      {message.options.map((option, index) => (
                        <button
                          ...
                          className="bg-yellow-400 hover:bg-yellow-500 text-black text-xs px-3 py-1 rounded-full transition-colors"
                        >
                          {option}
                        </button>
                      ))}
