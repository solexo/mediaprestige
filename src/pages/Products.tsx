...
const Products = React.memo(() => {
  ...
  return (
    <div className="min-h-screen bg-black">
      <Header />
      <section className="py-20 bg-gradient-to-b from-black via-gray-900 to-black pt-32">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {products.map((product, index) => (
              <div
                key={product.id}
                className="group relative bg-gradient-to-br from-gray-800 to-gray-900 rounded-2xl shadow-2xl hover:shadow-yellow-500/20 transition-all duration-500 hover:-translate-y-2 will-change-transform"
                ...
                <div className="relative h-64 overflow-hidden">
                  <Link to={`/products/${product.id}`}>
                    <img
                      ...
                      alt={product.name}
                      loading="lazy"
                      className={`w-full h-full ${product.id === 'l1-pro16' ? 'object-contain' : 'object-cover'} transform group-hover:scale-110 transition-transform duration-700`}
                    />
                  </Link>
                </div>
